...
class TestLiteLLMClientInit:
    ...
    def test_init_missing_api_key_raises_error(self, monkeypatch):
        """Client raises ValueError if api_key not provided or in environment."""
...
class TestErrorHandling:
    ...
    async def test_stream_completion_handles_api_error(self):
        ...
        with patch.object(
            ...
            with pytest.raises(Exception, match="API Error: Rate limited"):
                async for _ in client.stream_completion(
                    model="openai/gpt-4",
                    messages=[{"role": "user", "content": "Hi"}],
                ):
    ...
    async def test_stream_completion_logs_error(self):
        ...
        with patch("listeners._llm.logger") as mock_logger:
            with patch.object(
                client._client.chat.completions,
                "create",
                new_callable=AsyncMock,
